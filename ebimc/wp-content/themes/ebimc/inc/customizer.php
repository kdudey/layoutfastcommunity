...
function ebimc_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_section(
        'typography',
        array(
            'title' => __( 'Preset Styles', 'ebimc' ),
            //'description' => __( 'This is a section for the typography', 'ebimc' ),
            'priority' => 20,
        )
    );
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'header_bg_color',
            array(
                'label'      => __( 'Header Banner Background Color', 'ebimc' ),
    ...
    $wp_customize->add_setting( 'header_banner_tagline_setting', array(
        'default' => __( 'To customize the contents of this header banner and other elements of your site go to Dashboard - Appearance - Customize','ebimc' ),
        'sanitize_callback' => 'wp_filter_nohtml_kses',
    ) );
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_tagline_setting', array(
        'label' => __( 'Banner Tagline', 'ebimc' ),
        'section'    => 'header_image',
    ...
    $wp_customize->add_control( new WP_Customize_Control($wp_customize, 'header_banner_visibility', array(
        ...
        'label'    => __('Remove Header Banner', 'ebimc'),
        'section'    => 'header_image',
        'type'     => 'checkbox',
    ) ) );
   ...
   $wp_customize->add_section(
        ...
        array(
            'title' => __( 'Other Customizations', 'ebimc' ),
            //'description' => __( 'This is a section for the header banner Image.', 'ebimc' ),
            'priority' => 40,
        )
    );
    $wp_customize->add_section(
        'colors',
        array(
            'title' => __( 'Background Color', 'ebimc' ),
            //'description' => __( 'This is a section for the header banner Image.', 'ebimc' ),
            'priority' => 50,
    ...
    $wp_customize->add_section(
        'background_image',
        array(
            'title' => __( 'Background Image', 'ebimc' ),
            //'description' => __( 'This is a section for the header banner Image.', 'ebimc' ),
            'priority' => 60,
            'panel' => 'styling_option_panel',
        )
    ...
    $wp_customize->add_setting( 'ebimc_logo', array(
        //'default' => __( '', 'ebimc' ),
        'sanitize_callback' => 'esc_url',
    ) );
    $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'ebimc_logo', array(
        'label'    => __( 'Upload Logo (replaces text)', 'ebimc' ),
        'section'  => 'title_tagline',
        'settings' => 'ebimc_logo',
    ) ) );

}
add_action( 'customize_register', 'ebimc_customize_register' );

add_action( 'wp_head', 'ebimc_customizer_css');
function ebimc_customizer_css()
{
    ?>
    <style type="text/css">
 ...
 */
function ebimc_customize_preview_js() {
    wp_enqueue_script( 'ebimc_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'ebimc_customize_preview_js' );
